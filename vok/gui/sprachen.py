...
class gui_sprachen(Gtk.Window):
   def __init__(self,maingui,geometry,kartei): 
      ...
      width,height = int(180.0*max(1,geometry[0])),int(200.0*max(1,geometry[1]))
   ...
   def menuitem_cb(self,widget,string):
      if string == "rem_kap":
         ...
         kap = kap_add(self,self.kartei,self.spr_store[self.select.get_active()][1])
         if kap != None:
             self.refresh_kap()
             self.refreshed[0] = True
   ...
   def spr_button_cb(self,button,action):
      if action == "add":
         ...
         if spr_rem(self,self.kartei,self.spr_store[self.select.get_active()][1]):
            self.parent_gui.refresh_spr()
            self.refresh_view()
      elif action == "edit":
